...
def add_event(
    events_table: EventsTable, meanings_table: MeaningsTable, data: pd.DataFrame
) -> None:
    """
    ...
    data : pd.DataFrame
        A DataFrame containing new event data. Each row corresponds to a new event entry and contains
    ...
    for index, row in data.iterrows():
        ...
        meanings_table.add_row(
            value=row['name'],
            meaning=f"{row['name']} - Update description!!!", #TODO: get event description
        )
...
if __name__ == "__main__":
    ...
    for stream in streams:
        ...
        if isinstance(stream.parent, data_contract.HarpDevice):
            ...
            nwb_file.add_acquisition(pynwb.core.DynamicTable(name=stream.name, description=json.dumps(data)))
